...
'''
precise_data = pd.read_csv(r'/home/wwj/chenqiliang/SVD/newAll-channel_matrix_p_1.csv').iloc[:, 1:]
precise_data = np.asarray(precise_data, np.float32).reshape(-1, 8, 8, 1)
precise_label = pd.read_csv(r'/home/wwj/chenqiliang/SVD/newcapacity_labels_p_1.csv').iloc[:, 1].values.astype(np.int32)

noisy_data = pd.read_csv(r'/home/wwj/chenqiliang/SVD/newAll-channel_matrix_p_1.csv').iloc[:, 1:]
noisy_data = np.asarray(noisy_data, np.float32).reshape(-1, 8, 8, 1)
noisy_label = pd.read_csv(r'/home/wwj/chenqiliang/SVD/JTRAS_capacity_label_1.csv').iloc[:, 1].values.astype(np.int32)

combined_data = np.concatenate([precise_data, noisy_data], axis=0)
combined_label = np.concatenate([precise_label, noisy_label], axis=0)


weights = np.concatenate([np.full(160000, 2.), np.full(200000, 1.)])


n_class = 784
n_sample = combined_label.shape[0]
label_array = np.zeros((n_sample, n_class))
for i in range(n_sample):
    label_array[i, combined_label[i] - 1] = 1  


xTrain, xTest, yTrain, yTest = train_test_split(combined_data, label_array, test_size=0.1, random_state=40)
...
weights_tensor = torch.tensor(weights[:len(xTrain)], dtype=torch.float32)  

class WeightedDataset(TensorDataset):
    def __init__(self, data_tensor, label_tensor, weight_tensor):
        super().__init__(data_tensor, label_tensor)
...
train_dataset = WeightedDataset(xTrain_tensor, yTrain_tensor, weights_tensor)
...
for epoch in range(num_epochs):
    ...
    for inputs, labels, batch_weights in t:
        inputs, labels, batch_weights = inputs.to(device), labels.to(device), batch_weights.to(device)
        labels = torch.argmax(labels, dim=1)  
