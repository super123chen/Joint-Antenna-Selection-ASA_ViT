...
class SingleHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = torch.matmul(q, k.transpose(-2, -1)) / (self.embed_dim ** 0.5)  # (B, seq_len, seq_len)
        attn_weights = F.softmax(scores, dim=-1)  # (B, seq_len, seq_len)
        attn_weights = self.dropout(attn_weights)

        
        output = torch.matmul(attn_weights, v)  # (B, seq_len, embed_dim)
        return output
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=8, patch_size=8, in_chans=1, embed_dim=512):    ##################
...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim=512, num_heads=1, mlp_ratio=0.2, depth=1, qkv_bias=True,):    ##############
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList([
                   ...
                   #nn.MultiheadAttention(embed_dim, num_heads, bias=qkv_bias),
                  
                   SingleHeadAttention(embed_dim, dropout=0.),
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=8, patch_size=8, in_chans=1, num_classes=784, embed_dim=512, depth=1, num_heads=1,
                 mlp_ratio=0.1, qkv_bias=True,  ):   ###############################
